...
pub(crate) static BIG_1: LazyLock<BigUint> = LazyLock::new(BigUint::one);
pub(crate) static BIG_2: LazyLock<BigUint> = LazyLock::new(|| BigUint::from_u64(2).unwrap());
pub(crate) static BIG_64: LazyLock<BigUint> = LazyLock::new(|| BigUint::from_u64(64).unwrap());
...
/// The probability of returning true for a randomly chosen non-prime is at most 1/4^n.
///
...
pub fn probably_prime_miller_rabin(n: &BigUint, reps: usize, force2: bool) -> bool {
    ...
    let mut rng = StdRng::seed_from_u64(n.get_limb(0));
...
pub fn probably_prime_lucas(n: &BigUint) -> bool {
    ...
    // try increasing P >= 3 such that D = P^2 - 4 (so Q = 1)
    // until Jacobi(D, n) = -1.
    ...
    // (D, P, Q above have become delta, b, 1):
    //
    // Let U_n = U_n(b, 1), V_n = V_n(b, 1), and delta = b^2-4.
    // An extra strong Lucas pseudoprime to base b is a composite n = 2^r s + Jacobi(delta, n),
    // where s is odd and gcd(n, 2*delta) = 1, such that either (i) U_s = 0 mod n and V_s = +/-2 mod n,
    // or (ii) V_{2^t s} = 0 mod n for some 0 <= t < r-1.
    //
    // We know gcd(n, delta) = 1 or else we'd have found Jacobi(d, n) == 0 above.
    // We know gcd(n, 2) = 1 because n is odd.
    //
    // Arrange s = (n - Jacobi(delta, n)) / 2^r = (n+1) / 2^r.
    let mut s = n + &*BIG_1;
    ...
    //
    // In general V(k) = alpha^k + beta^k, where alpha and beta are roots of x^2 - Px + Q.
    ...
    //  V(2k+1) = V(k) V(k+1) - P
    //
    // We can therefore start with k=0 and build up to k=s in log2(s) steps.
    ...
    for _ in 0..r - 1 {
        ...
        // Optimization: V(k) = 2 is a fixed point for V(k') = V(k)^2 - 2,
...
fn get_bit(x: &BigUint, i: usize) -> u8 {
    let j = i / big_digit::BITS as usize;
    // if is out of range of the set words, it is always false.
    if i >= x.bits() as usize {
        return 0;
    }

    (x.get_limb(j) >> (i % big_digit::BITS as usize) & 1) as u8
}
...
mod tests {
    ...
    // Test Cases from #51
    const ISSUE_51: &[&str] = &["1579751", "1884791", "3818929", "4080359", "4145951"];
